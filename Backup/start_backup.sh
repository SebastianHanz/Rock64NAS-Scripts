...
copyAppData(){
		...
		echo -e "AppData-Backup erfolgreich beendet!\n"
}
...
copyIOBroker(){
		...
		echo -e "\nIoBroker-Backup erfolgreich beendet!\n"
}
...
copyOMVBACKUP(){
		...
		echo -e "\n OMVBACKUP-Backup erfolgreich beendet!\n"
}
...
infoALL()
{
   ...
   echo
     if [ "$1" = "-zip" ];
		then
		echo 
		echo ' Archivierung beginnt jetzt! Leider kann sieht man den Fortschritt nicht, also GEDULD bitte..'
...
if [[ "$*" == *"-zip"* ]];then
	...
	case "$1" in
	   -all)       	SOURCE="$SRC_APPDATA $SRC_IOBROKER $OMVBACKUP $SRC_SCRIPTS "
					infoALL -zip
	   ;;
	   -iobroker)  	SOURCE="$SRC_IOBROKER "
					infoARCHIVES 
	   ;;
	   -appdata)   	SOURCE="$SRC_APPDATA "
					infoARCHIVES 
	   ;;
	   -omvbackup) 	SOURCE="$OMVBACKUP "
					infoARCHIVES 
	   ;;
	   -scripts) 	SOURCE="$SRC_SCRIPTS "
					infoARCHIVES 
	   ;;
	   -zip)
	   ;;
