...
while True:
	try:
	 ...
	 print("CPU-Temperatur= {0}C".format(tempCPU)) #Gebe Temperatur in Console aus	

	 read= readBlock(address,0,4) #Lese 4Byte an Daten von USV in read-Array ein
	 w=open('/media/ramdisk/USV.txt' ,'w') #Oeffne die Txt-Datei in die die USV-Daten geschrieben werden sollen

	 #Schreibe empfangene daten von USV in Txt-Datei, nach angegebener Struktur
	 w.write('{0}\n{1}\n{2}\n{3}\n{4}'.format(data,float(read[0]),float(read[1]),float(read[2]),read[3]))
	 w.close()
