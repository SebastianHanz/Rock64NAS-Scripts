...
checkCHECKSUM() {
	...
	if [ "$FILE_SHA" = "$ZIP_SHA" ]; then
		...
		echo -e "\n Die Checksumen stimmen nicht ueberein! Es liegt wohl eine falsche Datei vor!\n ABBRUCH\n"
		return -1
		echo Finish!
		exit
	fi
...
#Lege Downloadordner fuer Nextcloud-Dateien an falls nicht vorhanden
if [ ! -d "$DIR_TEMP" ]; then
	cd /
	mkdir $DIR_TEMP
	cd /
	echo -e "Der Downloadordner fuer Nextcloud wurde angelegt!\n"
fi

#Wechsle in den Downloadordner
cd /$DIR_TEMP

#Ueberpruefe ob eine URL mit angegeben wurde
if [[ $URL == *"https://download.nextcloud.com/server/releases"* ]]; then #Wenn URL enthaelt
