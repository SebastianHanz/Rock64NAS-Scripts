...
if ( ifconfig | grep -q "eth0" ) ; then IP_LAN=`ifconfig eth0 | grep "inet" | cut -d " " -f 1 ` ; else IP_LAN="---" ; fi ;
#if ( ifconfig | grep -q "eth0" ) ; then IP_LAN=`ifconfig eth0 | grep "inet" ` ; else IP_LAN="---" ; fi ;
...
\033[1;31m( : '~'.~.'~' : ) \033[0;37mÿ Auslastung..: $LOAD1 (1 Min.) | $LOAD2 (5 Min.) | $LOAD3 (15 Min.)
